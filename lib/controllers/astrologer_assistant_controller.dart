import 'package:trueastrotalk/model/assistant_model.dart';
...
class AstrologerAssistantController extends GetxController {
  ...
  storeChatId(int partnerId) async {
    try {
      await global.checkBody().then((result) async {
        if (result) {
          await apiHelper.storeAssistantFirebaseChatId(global.user.id!, partnerId).then((result) {
            if (result.status == "200") {
              firebaseChatId = result.recordList['recordList'];
              update();
              print('chat id genrated:- $firebaseChatId');
            } else {
              global.showToast(message: '${result.status} problem to store firebase chat id', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
  ...
  Future<ChatMessageModel?> getLastMessages({String? chatId}) async {
    try {
      ...
      return mm.isNotEmpty ? mm[0] : ChatMessageModel(message: '', createdAt: DateTime.now().subtract(const Duration(days: 365)), url: '');
    } catch (err) {
  ...
  getChatWithAstrologerAssisteant() async {
    try {
      await global.checkBody().then((result) async {
        if (result) {
          await apiHelper.getAssistantChat().then((result) {
            if (result.status == "200") {
              ...
              global.showToast(message: '${result.status} problem to get assistant chat history', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
    ...
    try {
      await global.checkBody().then((result) async {
        if (result) {
          ...
          await apiHelper.checkAstrologerPaidSession(astrologerId).then((result) {
            if (result.status == "200") {
      ...
      await global.checkBody().then((result) async {
        if (result) {
          await apiHelper.blockAstrologerAssistant(assistantId).then((result) async {
            if (result.status == "200") {
              global.showToast(message: 'Assistant Block Successfully', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            } else {
              global.showToast(message: 'Fail block Assistant', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
        ...
        if (result) {
          await apiHelper.deleteAssistantChat(id).then((result) async {
            if (result.status == "200") {
              ...
              global.showToast(message: 'Assistant Deleted Successfully', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            } else {
              global.showToast(message: '${result.status} Fail Deleted Assistant', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
