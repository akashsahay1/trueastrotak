        ...
        if (result) {
          await apiHelper.orderAdd(productCatId: catId, productId: prodId, addressId: addId, gst: gstPercent, paymentMethod: payMethod, amount: payAmount, totalPay: totalPayment).then((result) async {
            if (result.status == "200") {
              ...
              global.showToast(message: 'Order SuccessFully', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            } else {
              global.showToast(message: 'Order not created', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
        ...
        if (result) {
          await apiHelper.addAddress(userAddressModel).then((result) {
            if (result.status == "200") {
              ...
              global.showToast(message: 'Address not added please try later!', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
    ...
    await global.checkBody().then((result) async {
      if (result) {
        await apiHelper.updateAddress(id, userAddressModel).then((result) {
          if (result.status == "200") {
            global.showToast(message: 'Your address have been updated', textColor: global.textColor, bgColor: global.toastBackGoundColor);
          } else {
            global.showToast(message: '', textColor: global.textColor, bgColor: global.toastBackGoundColor);
          }
        });
