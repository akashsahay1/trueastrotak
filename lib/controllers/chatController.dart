...
class ChatController extends GetxController with GetSingleTickerProviderStateMixin {
  ...
  var pdf = pw.Document();
  String? duration;
  CollectionReference userChatCollectionRefRTM = FirebaseFirestore.instance.collection("LiveChats");
  ...
  Future<void> sendReplyMessage(String message, String chatId, int partnerId, bool isEndMessage, String replymsg) async {
    debugPrint('chatID $chatId partnerId $partnerId');
    try {
      if (message.trim() != '') {
        ChatMessageModel chatMessage = ChatMessageModel(message: message, createdAt: DateTime.now(), updatedAt: DateTime.now(), isDelete: false, isRead: true, userId1: '${global.currentUserId}', userId2: '$partnerId', isEndMessage: isEndMessage, replymsg: replymsg);
        update();
  ...
  Future<void> updateProfileImage(String partnerId, String chatId, String imageUrl) async {
    ChatMessageModel chatMessageModel = ChatMessageModel(createdAt: DateTime.now(), updatedAt: DateTime.now(), isDelete: false, isRead: true, userId1: '${global.currentUserId}', userId2: '$partnerId', attachementPath: imageUrl, isEndMessage: false);
    // global.hideLoader();
    // Upload the message to Firestore
    await uploadMessage(chatId, '$partnerId', chatMessageModel);
  }

  Future<void> uploadImage(File imageFile, String partnerId, String chatId) async {
    try {
      final storageReference = FirebaseStorage.instance.ref().child('$chatId/${DateTime.now().millisecondsSinceEpoch}_${imageFile.path.split('/').last}');
  ...
  showBottomAcceptChatRequest({required int astrologerId, required int chatId, required String astroName, required String fcmToken, required String astroProfile, required String firebaseChatId, required String duration}) async {
    log("check");
        ...
        if (result) {
          await apiHelper.getAstrologerCategory().then((result) {
            if (result.status == "200") {
              categoryList.add(AstrologerCategoryModel(image: '', name: 'All', id: 1));
  ...
  sendChatRequest(int astrologerId, bool isFreeSession, String time) async {
    try {
      await apiHelper.sendAstrologerChatRequest(astrologerId, isFreeSession, time).then((result) {
        if (result.status == "200") {
          global.showToast(message: 'Sending chat request..', textColor: global.textColor, bgColor: global.toastBackGoundColor);
        } else {
          global.showToast(message: 'Failed to send chat request', textColor: global.textColor, bgColor: global.toastBackGoundColor);
        }
      });
    } catch (e) {
  ...
  Future<void> sendMessage(String message, String chatId, int partnerId, bool isEndMessage) async {
    try {
      if (message.trim() != '') {
        ChatMessageModel chatMessage = ChatMessageModel(message: message, createdAt: DateTime.now(), updatedAt: DateTime.now(), isDelete: false, isRead: true, userId1: '${global.currentUserId}', userId2: '$partnerId', isEndMessage: isEndMessage);
        update();
        await uploadMessage(chatId, '$partnerId', chatMessage);
      } else {}
    } catch (e) {
  ...
  uploadMessage(String idUser, String partnerId, ChatMessageModel msgModel) async {
    try {
  ...
  rejectedChat(String cid) async {
    try {
      await apiHelper.rejectChat(cid).then((result) {
        if (result.status == "200") {
          global.showToast(message: 'Chat Rejected', textColor: global.textColor, bgColor: global.toastBackGoundColor);
        } else {
          global.showToast(message: 'Chat Reject fail', textColor: global.textColor, bgColor: global.toastBackGoundColor);
        }
      });
    } catch (e) {
  ...
  acceptedChat(int chatId) async {
    try {
      await apiHelper.acceptChat(chatId).then((result) {
        if (result.status == "200") {
          global.showToast(message: 'Chat Accepted', textColor: global.textColor, bgColor: global.toastBackGoundColor);
        } else {
          global.showToast(message: 'Chat Accepet fail', textColor: global.textColor, bgColor: global.toastBackGoundColor);
        }
      });
    } catch (e) {
  ...
  addReview(int astrologerId) async {
    try {
      await global.checkBody().then((result) async {
        if (result) {
          await apiHelper.addAstrologerReview(astrologerId, rating, reviewController.text, !isPublic).then((result) async {
            if (result.status == "200") {
              ...
              global.showToast(message: 'Failed to add review', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
      ...
      await global.checkBody().then((result) async {
        if (result) {
          await apiHelper.deleteReview(reviewId).then((result) async {
            if (result.status == "200") {
              global.showToast(message: 'Your review has been deleted', textColor: global.textColor, bgColor: global.toastBackGoundColor);
              reviewData.clear();
              rating = 0;
              reviewController.clear();
              update();
            } else {
              global.showToast(message: 'Delete review failed!', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
  ...
  updateReview(int reviewId, int astrologerId) async {
    try {
      var basicDetails = {"rating": rating, "review": reviewController.text == "" ? "" : reviewController.text, "astrologerId": astrologerId, "isPublic": !isPublic};
      await global.checkBody().then((result) async {
        if (result) {
          await apiHelper.updateAstrologerReview(reviewId, basicDetails).then((result) async {
            if (result.status == "200") {
              global.showToast(message: 'Your review has been updated', textColor: global.textColor, bgColor: global.toastBackGoundColor);
              await getuserReview(astrologerId);
              Get.back();
            } else {
              global.showToast(message: 'Failed to update review', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>>? getMessageRTM(String channelID) {
    try {
      Stream<QuerySnapshot<Map<String, dynamic>>> data = userChatCollectionRefRTM.doc(channelID).collection('messages').orderBy('createdAt').snapshots();
      return data;
    } catch (err) {
  ...
  Future uploadMessageRTM(String idUser, MessageModel anonymous) async {
    ...
    try {
      final refMessages = userChatCollectionRefRTM //SEND BY CURRENT USER
          .doc(idUser)
          .collection('messages');
      var messageResult = await refMessages.add(anonymous.toJson()).catchError((e) {
        log('send mess exception' + e);
      });

      return {'user1': messageResult.id};
    } catch (err) {
      log('uploadMessage err $err');
    }
  }
