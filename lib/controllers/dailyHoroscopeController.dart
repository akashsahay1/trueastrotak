        ...
        if (result) {
          await apiHelper.getHoroscope(horoscopeSignId: horoscopeId).then((result) {
            if (result != null) {
              ...
              update();
              dailyhoroscopeData!['astroApiCallType'].toString() == "2" ? dailyList = DailyscopeModel.fromJson(dailyhoroscopeData!['recordList']) : null;
              update();
            } else {}
          });
        }
      ...
      await global.checkBody().then((result) async {
        if (result) {
          ...
          await apiHelper.addFeedBack(feedbackType!, feedback).then((result) {
            if (result.status == "200") {
              global.showToast(message: 'Thanks for your feedback!', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            } else {
              if (global.currentUserId != null) {
                global.showToast(message: 'Fail to add feedback please try again later!', textColor: global.textColor, bgColor: global.toastBackGoundColor);
              }
            }
          });
        }
