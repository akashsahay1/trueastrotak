      ...
      await global.checkBody().then((result) async {
        if (result) {
          await apiHelper.followAstrologer(astrologerId).then((result) {
            if (result.status == "200") {
              global.showToast(message: 'You will be recived notifed when Astrologer is online and live !', textColor: global.textColor, bgColor: global.toastBackGoundColor);
              isFollowed = true;
              update();
              ...
              if (global.currentUserId != null) {
                global.showToast(message: 'Astrologer not followed please try again later', textColor: global.textColor, bgColor: global.toastBackGoundColor);
              } else {
                global.showToast(message: 'You have to login to follow astrologer', textColor: global.textColor, bgColor: global.toastBackGoundColor);
              }
            }
          });
        }
      ...
      if (result) {
        await apiHelper.unFollowAstrologer(astrologerId).then((result) async {
          if (result.status == "200") {
            ...
            global.showToast(message: 'Unfollow failed please try again later', textColor: global.textColor, bgColor: global.toastBackGoundColor);
          }
        });
      }
    });
        ...
        if (result) {
          await apiHelper.getFollowedAstrologer(startIndex, fetchRecord).then((result) async {
            if (result.status == "200") {
              ...
              if (isLogin) {
                global.showToast(message: 'Failed to get follower', textColor: global.textColor, bgColor: global.toastBackGoundColor);
              }
            }
          });
        }
