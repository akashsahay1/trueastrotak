...
class KundliController extends GetxController with GetSingleTickerProviderStateMixin {
  ...
  String prefix = '';
  List<KundliGender> gender = [KundliGender(title: 'Male', isSelected: false, image: Images.male), KundliGender(title: 'Female', isSelected: false, image: Images.female), KundliGender(title: 'Other', isSelected: false, image: Images.otherGender)];
  int initialIndex = 0;
  ...
  List<Kundli> listIcon = [Kundli(icon: Icons.person, isSelected: true), Kundli(icon: Icons.search, isSelected: false), Kundli(icon: Icons.calendar_month, isSelected: false), Kundli(icon: Icons.punch_clock_outlined, isSelected: false), Kundli(icon: Icons.location_city, isSelected: false)];
          ...
          await apiHelper.getKundli().then((result) {
            if (result.status == "200") {
              ...
              if (global.currentUserId != null) {
                global.showToast(message: 'FAil to get kundli', textColor: global.textColor, bgColor: global.toastBackGoundColor);
              }
            }
          });
        }
  ...
  addKundliData(String pdfType, int amount) async {
    List<KundliModel> kundliModel = [
      KundliModel(
        ...
        forMatch: 0,
        lang:
            dropDownController.kundaliLang.toString() == "English"
                ? 'en'
                    ...
                    : (dropDownController.kundaliLang.toString() == "Kannada"
                        ...
                        : (dropDownController.kundaliLang.toString() == "Telugu"
                            ...
                            : (dropDownController.kundaliLang.toString() == "Hindi"
                                ? 'hi'
                                : (dropDownController.kundaliLang.toString() == "Malayalam" ? 'ml' : (dropDownController.kundaliLang.toString() == "Spanish" ? 'sp' : (dropDownController.kundaliLang.toString() == "French" ? 'fr' : 'en'))))))),
      ),
    ];
      ...
      if (result) {
        await apiHelper.addKundli(kundliModel, amount, false).then((result) {
          if (result.status == "200") {
            ...
            global.showToast(message: 'Failed to create kundli please try again later!', textColor: global.textColor, bgColor: global.toastBackGoundColor);
          }
        });
      }
    });
  ...
  getGeoCodingLatLong({double? latitude, double? longitude, int? flagId, KundliMatchingController? kundliMatchingController}) async {
    try {
      await global.checkBody().then((result) async {
        if (result) {
          await apiHelper.geoCoding(lat: latitude, long: longitude).then((result) {
            if (result.status == "true") {
              if (flagId == 1) {
                kundliMatchingController!.boyTimezone = double.parse(result.recordList['timezone'].toString());
                kundliMatchingController.update();
              } else if (flagId == 2) {
                kundliMatchingController!.girlTimezone = double.parse(result.recordList['timezone'].toString());
                kundliMatchingController.update();
              } else {
  ...
  updateKundliData(int id) async {
    KundliModel kundliModel = KundliModel(name: editNameController.text, gender: dropDownGender!, birthDate: pickedDate ?? editDOB, birthTime: editBirthTimeController.text, birthPlace: editBirthPlaceController.text, latitude: lat, longitude: long);
    update();
    await global.checkBody().then((result) async {
      if (result) {
        await apiHelper.updateKundli(id, kundliModel).then((result) {
          if (result.status == "200") {
            global.showToast(message: 'Your kundli has been updated', textColor: global.textColor, bgColor: global.toastBackGoundColor);
          } else {
            global.showToast(message: 'Failed to update kundli please try again later!', textColor: global.textColor, bgColor: global.toastBackGoundColor);
          }
        });
      }
    });
      ...
      if (result) {
        await apiHelper.deleteKundli(id).then((result) {
          if (result.status == "200") {
            ...
            global.showToast(message: 'Deleted Fail', textColor: global.textColor, bgColor: global.toastBackGoundColor);
          }
        });
      }
    });
        ...
        if (result) {
          await apiHelper.getKundliBasicPanchangDetails(day: day, month: month, year: year, hour: hour, min: min, lat: lat, lon: lon, tzone: tzone).then((result) {
            if (result != null) {
              ...
              global.showToast(message: 'Fail to getKundliBasicPanchangDetails', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
            update();
          });
        }
