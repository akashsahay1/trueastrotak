      ...
      if (result) {
        await apiHelper.addKundli(kundliModel, 0, true).then((result) async {
          if (result.status == "200") {
            try {
              final response = await http.post(Uri.parse('$baseUrl/KundaliMatching/report'), headers: await global.getApiHeaders(true), body: json.encode({"male_kundli_id": result.recordList[0]['id'].toInt(), "female_kundli_id": result.recordList[1]['id'].toInt()}));
              dynamic recordList1;
              print("${response.statusCode}");
              if (response.statusCode == 200) {
                log("${response.body}");
                direction == "South" ? southKundaliMatchingModel = SouthKundaliMatchingModel.fromJson(jsonDecode(response.body)) : northKundaliMatchingModel = NorthKundaliMatchingModel.fromJson(jsonDecode(response.body));
                global.hideLoader();
                update();
                update();
              } else {
