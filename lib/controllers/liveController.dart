...
class LiveController extends GetxController {
  ...
  Future uploadMessage(String idUser, int astrologerId, MessageModelLive anonymous) async {
    try {
      ...
      return {'user1': messageResult.id, 'user2': message1Result.id};
    } catch (err) {
        ...
        if (result) {
          ...
          await apiHelper.getWaitList(channel).then((result) {
            ...
            if (result.status == "200") {
              ...
              global.showToast(message: 'Fail to getWaitList', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
        ...
        if (result) {
          ...
          await apiHelper.updateStatusForWaitList(id: id, status: status).then((result) {
            ...
            if (result.status == "200") {
              ...
              global.showToast(message: 'Fail to updateStatusForWaitList', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
        ...
        if (result) {
          await apiHelper.cutPaymentForLiveStream(userId, astrologerId, timeInSecond, transactionType, chatId, sId1: sId1, sId2: sId2, channelName: channelName).then((result) {
            if (result.status == "200") {
              ...
              global.showToast(message: 'Fail to cut payment', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
        ...
        if (result) {
          await apiHelper.deleteFromWishList(id).then((result) {
            if (result.status == "200") {
              ...
              global.showToast(message: 'Fail to deleteFromWishList', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Container(
            ...
            child: Column(
              children: [
                CircleAvatar(radius: 30, backgroundColor: Get.theme.primaryColor, child: Center(child: Icon(Icons.phone, color: Colors.black, size: 35))),
                Padding(padding: EdgeInsets.only(top: 10), child: Text("$astroName ${tr("is available for call")}", style: TextStyle(color: Colors.grey, fontSize: 14), textAlign: TextAlign.center)),
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      GestureDetector(
                        ...
                        child: Container(height: 40, width: 100, decoration: BoxDecoration(color: Colors.grey, borderRadius: BorderRadius.circular(50)), child: Center(child: Text("Decline", style: TextStyle(color: Colors.white, fontSize: 13)).tr())),
                      ),
                      GestureDetector(
                        onTap: () async {
                          ...
                          if (requestType != "Chat") {
                            ...
                            debugPrint("liveController.liveUsers " + liveController.liveUsers.toString());
                            ...
                            debugPrint("otherJoinUsersFcmTokens" + otherJoinUsersFcmTokens.toString());
                            await global.callOnFcmApiSendPushNotifications(
                              fcmTokem: otherJoinUsersFcmTokens,
                              title: "For starting the timer in other audions for video and audio",
                              subTitle: "For starting the timer in other audions for video and audio",
                              waitListId: "${id}",
                              channelname: channel.toString(),
                              profile: global.user.profile.toString(),
                              name: global.user.name ?? "user",
                            ...
                            update();
                            global.callOnFcmApiSendPushNotifications(fcmTokem: ["$astrologerFcmToken"], title: "For Live Streaming Chat", subTitle: "For Live Streaming Chat", waitListId: "${id}", liveChatSUserName: global.user.name, sessionType: "start", chatId: chatId);
                            timer2 = Timer.periodic(Duration(seconds: 1), (timer) {
                              debugPrint("totalCompletedTimeForChat:" + totalCompletedTimeForChat.toString());
  ...
  Future<dynamic> addToWaitList(String channel, String requestType, int astrologerId, String time) async {
    try {
      await global.checkBody().then((result) async {
        if (result) {
          global.showOnlyLoaderDialog(Get.context);
          ...
          await apiHelper.addToWaitlist(channel: channel, requestType: requestType, time: "${int.parse(time.toString()) * 60}", userId: global.currentUserId, userName: "${global.user.name}", userProfile: "${global.user.profile}", userFcmToken: "$fcmToken", astrologerId: astrologerId).then((
            result,
          ) async {
            global.hideLoader();
            if (result.status == "200") {
    ...
    try {
      await global.checkBody().then((result) async {
        if (result) {
          debugPrint(global.user.name);
          String? fcmToken = await FirebaseMessaging.instance.getToken();
          LiveUserModel liveUserModel = LiveUserModel(fcmToken: fcmToken, channelName: channelName);
          await apiHelper.saveLiveUsers(liveUserModel).then((result) async {
            if (result.status == "200") {
              debugPrint('live user added successfully');
            } else {
              global.showToast(message: 'failed to add live user', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
        ...
        if (result) {
          await apiHelper.getLiveUsers(channel).then((result) {
            if (result.status == "200") {
              ...
              global.showToast(message: 'failed to get live users', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
        ...
        if (result) {
          await apiHelper.deleteLiveUsers().then((result) {
            if (result.status == "200") {
              ...
              global.showToast(message: 'failed to remove live users', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
        ...
        if (result) {
          await apiHelper.generateRtmToken(appId, appCertificate, chatId, channelName).then((result) {
            if (result.status == "200") {
              ...
              global.showToast(message: 'failed to get live RTM Token', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
      ...
      await global.checkBody().then((result) async {
        if (result) {
          ...
          await apiHelper.endLiveSession(astrologerId).then((result) async {
            global.hideLoader();
            if (result.status == "200") {
              global.showToast(message: 'Live session end', textColor: global.textColor, bgColor: global.toastBackGoundColor);
              ...
              global.showToast(message: 'End live session fail', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
