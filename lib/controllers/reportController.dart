...
class ReportController extends GetxController {
  ...
  List title = ['2022 Detailed yearly Report', 'Education Report', 'Love & Relationship Report', 'Child NameReport', 'Match Making Report', 'Travel & Forgeign Settlement', 'Child Birth Report', 'Ask any question on one topic', 'career Rerport', 'Business Report', 'Buiness Partner Loyalty Report'];
        ...
        if (result) {
          await apiHelper.getAstrologer(sortingKey: sorting).then((result) {
            if (result.status == "200") {
              ...
              global.showToast(message: 'Sorting Applied', textColor: global.textColor, bgColor: global.toastBackGoundColor);
              update();
            } else {
              global.showToast(message: 'Get Atrologer sorting failed', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
        ...
        if (result) {
          await apiHelper.getReportType(searchString, startIndex, fetchRecord).then((result) {
            if (result.status == "200") {
              ...
              global.showToast(message: '${result.status} get Report', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
      ...
      await global.checkBody().then((result) async {
        if (result) {
          await apiHelper.addReportIntakeDetail(getReportModelData).then((result) async {
            if (result.status == "200") {
              global.showToast(message: 'Add Form Data successfully', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            } else {
