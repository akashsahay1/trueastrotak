...
class ReviewController extends GetxController {
          ...
          await apiHelper.getReview(id).then((result) {
            if (result.status == "200") {
              ...
              if (global.currentUserId != null) {
                global.showToast(message: 'Failed to get review', textColor: global.textColor, bgColor: global.toastBackGoundColor);
              }
            }
          });
        }
  ...
  onShareWhatsappTap() async {
    ...
    if (await canLaunchUrl(whatsappAndroid)) {
      ...
      global.showToast(message: 'Please install whatsapp first', textColor: global.textColor, bgColor: global.toastBackGoundColor);
    }
  }
        ...
        if (result) {
          await apiHelper.blockAstrologerProfileReview(id, isBlocked, isReported).then((result) async {
            if (result.status == "200") {
              ...
              if (isBlocked != null) {
                global.showToast(message: 'Astrologer Review Blocked', textColor: global.textColor, bgColor: global.toastBackGoundColor);
              } else {
                global.showToast(message: 'Astrologer Review Reported', textColor: global.textColor, bgColor: global.toastBackGoundColor);
              }
            } else {
              global.showToast(message: ' Fail to block Astrologer Review', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
