...
import 'package:trueastrotalk/model/geoCodingModel.dart';
...
class SearchControllerCustom extends GetxController {
  ...
  List<SearchModel> searchData = [SearchModel(title: 'Astrologer', isSelected: true), SearchModel(title: 'Shopping', isSelected: false)];
        ...
        if (result) {
          await apiHelper.searchProductByCategory(productCategoryId, searchString).then((result) {
            if (result.status == "200") {
              ...
              global.showToast(message: 'Fail get product search result', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
