        ...
        if (result) {
          await apiHelper.getSkill().then((result) {
            if (result.status == "200") {
              ...
              global.showToast(message: 'Failed to get skill', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
        ...
        if (result) {
          await apiHelper.getAstrologer(sortingKey: sortBy, skills: skills, language: language, gender: gender, startIndex: bottomNavigationController.startIndex, fetchRecords: fetchRecord, catId: catId).then((result) {
            if (result.status == "200") {
              ...
              global.showToast(message: 'Failed to add filter', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
            update();
          });
        }
