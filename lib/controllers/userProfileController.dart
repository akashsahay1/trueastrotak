        ...
        if (result) {
          await apiHelper.updateUserProfile(id, basicDetails).then((result) async {
            if (result.status == "200") {
              global.showToast(message: 'Your Profile has been updated', textColor: global.textColor, bgColor: global.toastBackGoundColor);
              await splashController.getCurrentUserData();
              Get.back();
            } else {
              global.showToast(message: 'Failed to update profile please try again later!', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
      ...
      await global.checkBody().then((result) async {
        if (result) {
          await apiHelper.updetUserProfilePic(profile).then((result) async {
            if (result.status == "200") {
              global.showToast(message: 'Your Profile pic has been updated', textColor: global.textColor, bgColor: global.toastBackGoundColor);
              await splashController.getCurrentUserData();
              Get.back();
            } else {
              global.showToast(message: 'Failed to update profile please try again later!', textColor: global.textColor, bgColor: global.toastBackGoundColor);
        ...
        if (result) {
          ...
          await apiHelper.getZodiacProfileImg().then((result) async {
            ...
            if (result.status == "200") {
              ...
              global.showToast(message: 'Fail to Get defualt img!', textColor: global.textColor, bgColor: global.toastBackGoundColor);
            }
          });
        }
      });
