...
import 'package:trueastrotak/firebase_options.dart';
import 'package:trueastrotak/theme/nativeTheme.dart';
import 'package:trueastrotak/utils/CallUtils.dart';
import 'package:trueastrotak/utils/FallbackLocalizationDelegate.dart';
import 'package:trueastrotak/utils/binding/networkBinding.dart';
...
import 'package:trueastrotak/views/call/accept_call_screen.dart';
import 'package:trueastrotak/views/call/incoming_call_request.dart';
import 'package:trueastrotak/views/call/oneToOneVideo/onetooneVideo.dart';
import 'package:trueastrotak/views/chat/chat_screen.dart';
import 'package:trueastrotak/views/chat/incoming_chat_request.dart';
import 'package:trueastrotak/views/live_astrologer/live_astrologer_screen.dart';
import 'package:trueastrotak/views/splashScreen.dart';
...
Future<void> _firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  if (global.sp!.getString("currentUser") != null) {
    ...
    if (message.data["title"] == "For starting the timer in other audions for video and audio") {
    ...
    } else if (message.data["title"] == "For accepting time while user already splitted") {
      Future.delayed(Duration(milliseconds: 500)).then((value) async {
        await _localNotifications.cancelAll();
      });
      ...
      liveController.update();
    } else if (message.data["title"] == "Notification for customer support status update") {
      Future.delayed(Duration(milliseconds: 500)).then((value) async {
        await _localNotifications.cancelAll();
      });
...
AndroidNotificationChannel channel = const AndroidNotificationChannel('True Astrotalk local notifications', 'High Importance Notifications for Atroguru', importance: Importance.defaultImportance);
...
class _MyAppState extends State<MyApp> {
  ...
  void initState() {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      ...
      } else if (message.data["title"] == "For starting the timer in other audions for video and audio") {
        if (liveController.isImInLive == true) {
          int waitListId = int.parse(message.data["waitListId"].toString());
          String channelName = message.data['channelName'];
          liveController.joinUserName = message.data['name'] ?? "User";
                      ...
                      content: Container(
                        ...
                        child: Column(
                          children: [
                            CircleAvatar(
                              ...
                              child:
                                  messageData["profile"] == ""
                                      ...
                                      : CachedNetworkImage(
                                        imageUrl: '${global.imgBaseurl}${messageData["profile"]}',
                            ...
                            Padding(padding: EdgeInsets.only(top: 10), child: Text("${message.data["title"]}", style: TextStyle(color: Colors.grey, fontSize: 14), textAlign: TextAlign.center)),
                            Padding(
                                ...
                                children: [
                                  GestureDetector(
                                    onTap: () async {
                                      ...
                                      await chatController.rejectedChat(messageData["chatId"].toString());
                                      global.hideLoader();
                                      global.callOnFcmApiSendPushNotifications(fcmTokem: [messageData["fcmToken"]], title: 'End chat from customer');
                                      BottomNavigationController bottomNavigationController = Get.find<BottomNavigationController>();
                                      bottomNavigationController.setIndex(0, 0);
                                      Get.back();
                                      Get.to(() => BottomNavigationBarScreen(index: 0));
                                    },
                                    child: Container(height: 40, width: 100, decoration: BoxDecoration(color: Colors.grey, borderRadius: BorderRadius.circular(50)), child: Center(child: Text("Reject", style: TextStyle(color: Colors.white, fontSize: 13)).tr())),
                                  ),
                                  GestureDetector(
                                    onTap: () async {
                                      await _localNotifications.cancelAll();
                                      global.showOnlyLoaderDialog(context);
                                      await chatController.acceptedChat(int.parse(messageData["chatId"].toString()));

                                      global.callOnFcmApiSendPushNotifications(fcmTokem: [messageData["fcmToken"]], title: 'Start simple chat timer');
                                      global.hideLoader();
                                      ...
                                      Get.to(
                                        () => AcceptChatScreen(
                                          flagId: 1,
                                          astrologerName: messageData["astrologerName"] == null ? "Astrologer" : messageData["astrologerName"],
                                          profileImage: messageData["profile"] == null ? "" : messageData["profile"].toString(),
                                    ...
                                    child: Container(height: 40, width: 100, decoration: BoxDecoration(color: Colors.red, borderRadius: BorderRadius.circular(50)), child: Center(child: Text("Accept", style: TextStyle(color: Colors.white, fontSize: 13)).tr())),
                                  ),
                                ],
                              ),
                            ),
    ...
    return ResponsiveSizer(
      builder: (context, orientation, deviceType) {
        return GetMaterialApp(
          ...
          localizationsDelegates: [...context.localizationDelegates, FallbackLocalizationDelegate()],
  ...
  FlutterCallkitIncoming.onEvent.listen((CallEvent? event) async {
    ...
    switch (event.event) {
      ...
      case Event.actionCallDecline:
        ...
        global.callOnFcmApiSendPushNotifications(fcmTokem: [event.body['extra']["fcmToken"]], title: 'End chat from customer');
        callController.update();
        await chatController.rejectedChat(event.body['extra']["callId"].toString());
        callController.update();
  ...
  if (event.body['extra']['call_type'] == 10) {
    ...
    Get.to(
      () => AcceptCallScreen(
        ...
        astrologerName: event.body['extra']["astrologerName"] == null ? "Astrologer" : event.body['extra']["astrologerName"],
        astrologerProfile: event.body['extra']["profile"] == null ? "" : event.body['extra']["profile"],
    ...
    Get.to(() => OneToOneLiveScreen(channelname: event.body['extra']["channelName"], callId: event.body['extra']["callId"], fcmToken: event.body['extra']["token"].toString(), end_time: event.body['extra']['call_duration'].toString()));
  }
}

///custom notification
Future<void> foregroundNotificatioCustomAuddio(RemoteMessage payload) async {
  final initializationSettingsDarwin = DarwinInitializationSettings(defaultPresentBadge: true, requestSoundPermission: true, requestBadgePermission: true, defaultPresentSound: false);
  ...
  final initialSetting = InitializationSettings(android: android, iOS: initializationSettingsDarwin);
  ...
  AndroidNotificationDetails androidDetails = const AndroidNotificationDetails('channel_id_17', 'channel.name', importance: Importance.max, icon: "@mipmap/ic_launcher", playSound: true, enableVibration: true, sound: RawResourceAndroidNotificationSound('app_sound'));

  final iOSDetails = DarwinNotificationDetails(sound: customSound);
  final platformChannelSpecifics = NotificationDetails(android: androidDetails, iOS: iOSDetails);
  global.sp = await SharedPreferences.getInstance();
...
Future<void> foregroundNotification(RemoteMessage payload, String imageUrl) async {
  ...
  final initializationSettingsDarwin = DarwinInitializationSettings(defaultPresentBadge: true, requestSoundPermission: true, requestBadgePermission: true, defaultPresentSound: true);
  ...
  global.sp = await SharedPreferences.getInstance();
  if (global.sp!.getString("currentUser") != null) {
    await flutterLocalNotificationsPlugin.show(0, payload.data["title"], payload.data["description"], platformChannelSpecifics, payload: json.encode(payload.data.toString()));
  }
}
    ...
    try {
      ...
      if (body["notificationType"] == 1) {
        ...
        body['call_type'].toString() == "11"
            ? Get.to(() => OneToOneLiveScreen(channelname: body["channelName"], callId: body["callId"], fcmToken: body["token"], end_time: body['call_duration'].toString()))
            : Get.to(
              () => IncomingCallRequest(
        ...
        bool isFollow = bottomController.astrologerbyId[0].isFollow!;
        liveController.update();
        Get.to(() => LiveAstrologerScreen(token: token, channel: channelName, astrologerName: astrologerName, astrologerId: astrologerId, isFromHome: true, charge: charge, isForLiveCallAcceptDecline: false, videoCallCharge: videoCallCharge, isFollow: isFollow));
      } else {
      ...
      print('Exception in onSelectNotification main.dart:- ${e.toString()}');
    }
  }
}
