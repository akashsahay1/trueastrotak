...
class BlockAstrologerScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: PreferredSize(preferredSize: Size.fromHeight(56), child: CommonAppBar(title: 'Blocked Astrologer')),
        ...
        child: GetBuilder<SettingsController>(
          builder: (settingsController) {
            return settingsController.blockedAstroloer.isEmpty
                ? Center(child: Text("You have not Blocked any astrologer yet!").tr())
                : ListView.builder(
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Text('${settingsController.blockedAstroloer[index].astrologerName}'),
                                      Text('${settingsController.blockedAstroloer[index].allSkill}', style: Get.theme.primaryTextTheme.bodySmall!.copyWith(fontWeight: FontWeight.w300, color: Colors.grey[600])),
                                      Text('${settingsController.blockedAstroloer[index].languageKnown}', style: Get.theme.primaryTextTheme.bodySmall!.copyWith(fontWeight: FontWeight.w300, color: Colors.grey[600])),
                                      Text('${tr("Experience")} : ${settingsController.blockedAstroloer[index].experienceInYears} ${tr("Years")}', style: Get.theme.primaryTextTheme.bodySmall!.copyWith(fontWeight: FontWeight.w300, color: Colors.grey[600])),
                                    ],
                                  ),
                                ),
                              ),
                              Column(
                                children: [
                                  SizedBox(height: 20),
                                  TextButton(
                                    style: ButtonStyle(padding: WidgetStateProperty.all(EdgeInsets.all(0)), fixedSize: WidgetStateProperty.all(Size.fromWidth(90)), shape: WidgetStateProperty.all(RoundedRectangleBorder(borderRadius: BorderRadius.circular(10), side: BorderSide(color: Colors.red)))),
                                    onPressed: () async {
                                      Get.dialog(
                                        AlertDialog(
                                          backgroundColor: Colors.white,
                                          title: Column(
                                            crossAxisAlignment: CrossAxisAlignment.start,
                                            children: [Text("Unblock", style: Get.textTheme.titleMedium).tr(), Text("${tr("Are you sure you want to Unblock")} ${settingsController.blockedAstroloer[index].astrologerName} ?", style: Get.textTheme.bodyMedium)],
                                          ),
                                    ...
                                    child: Text('Unblock', style: Get.theme.primaryTextTheme.bodySmall!.copyWith(color: Colors.red)).tr(),
                                  ),
                                ],
                              ),
                            ],
