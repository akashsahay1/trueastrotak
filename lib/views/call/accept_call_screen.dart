...
class AcceptCallScreen extends StatefulWidget {
  ...
  String? duration;
  AcceptCallScreen({super.key, this.astrologerName, this.callId, this.astrologerId, this.astrologerProfile, this.token, this.callChannel, this.duration, this.isfromnotification = false});
...
class _AcceptCallScreenState extends State<AcceptCallScreen> {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        global.showToast(message: tr('Please leave the call by pressing leave button'), textColor: global.textColor, bgColor: global.toastBackGoundColor);
        return false;
      },
      child: Scaffold(
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Center(child: Column(children: [Text(widget.astrologerName ?? 'User', style: Get.textTheme.titleMedium!.copyWith(fontWeight: FontWeight.w500, fontSize: 20, color: Colors.white)), SizedBox(child: status()), const SizedBox(height: 15)])),
                ],
              ),
            ),
            Expanded(
              child: Center(
                child: Container(
                  ...
                  child: CircleAvatar(
                    ...
                    radius: 60,
                    child:
                        widget.astrologerProfile == null
                            ? Image.asset(Images.deafultUser, fit: BoxFit.cover)
                            : CachedNetworkImage(
                              height: 12.h,
                              width: 12.h,
                              imageUrl: '${global.imgBaseurl}${widget.astrologerProfile}',
                              imageBuilder: (context, imageProvider) => CircleAvatar(radius: 5.h, backgroundColor: Colors.transparent, child: Image.network(height: 12.h, width: 12.h, '${global.imgBaseurl}${widget.astrologerProfile}', fit: BoxFit.cover)),
                              placeholder: (context, url) => const Center(child: CircularProgressIndicator()),
                              errorWidget: (context, url, error) => Image.asset(Images.deafultUser, fit: BoxFit.contain, height: 60, width: 40),
                            ),
                  ),
                ),
              ),
          ...
          child: Row(
            ...
            children: [
              InkWell(
                ...
                child: Icon(Icons.volume_up, color: isSpeaker ? Colors.blue : Colors.white),
              ),
              InkWell(
                onTap: () async {
                  print("jaksndkjnsa");
                ...
                child: Container(padding: const EdgeInsets.all(10), margin: const EdgeInsets.only(right: 20), decoration: BoxDecoration(color: Colors.red, borderRadius: BorderRadius.circular(30)), child: Icon(Icons.call_end, color: Colors.white)),
              ),
              InkWell(
                onTap: () {
                  setState(() {
                ...
                child: Icon(Icons.mic_off, color: isMuted ? Colors.blue : Colors.white),
              ),
            ],
          ),
        ),
  ...
  Widget status() {
    return endTime == null
        ? Text("Joining.. ", style: TextStyle(fontWeight: FontWeight.w500, color: Colors.white, fontSize: 16.sp))
        : CountdownTimer(
          endTime: endTime,
          widgetBuilder: (_, CurrentRemainingTime? time) {
            if (time == null) {
              return Text('00:00:00', style: TextStyle(fontWeight: FontWeight.w500, color: Colors.white, fontSize: 16.sp));
            }
            return Padding(
              padding: const EdgeInsets.only(left: 10),
              child:
                  time.hours != null && time.hours != 0
                      ? Text('${time.hours ?? '00'} :${time.min! <= 9 ? '0${time.min}' : time.min ?? '00'} :${time.sec! <= 9 ? '0${time.sec}' : time.sec}', style: TextStyle(fontWeight: FontWeight.w500, color: Colors.white, fontSize: 16.sp))
                      : time.min != null
                      ? Text('${time.min! <= 9 ? '0${time.min}' : time.min ?? '00'} :${time.sec! <= 9 ? '0${time.sec}' : time.sec}', style: TextStyle(fontWeight: FontWeight.w500, color: Colors.white, fontSize: 16.sp))
                      : Text('${time.sec! <= 9 ? '0${time.sec}' : time.sec}', style: TextStyle(fontWeight: FontWeight.w500, color: Colors.white, fontSize: 16.sp)),
            );
          },
          onEnd: () async {
  ...
  void join() async {
    ...
    ChannelMediaOptions options = const ChannelMediaOptions(clientRoleType: ClientRoleType.clientRoleBroadcaster, channelProfile: ChannelProfileType.channelProfileCommunication);
    print('in join method of customer call');
    print('token:- ${widget.token!}');
    print('token:- ${widget.callChannel!}');
    print('token:- ${uid}');
    await agoraEngine.joinChannel(token: widget.token!, channelId: widget.callChannel!, options: options, uid: uid);
  }
  ...
  Future stopRecord() async {
    ...
    await callController.agoraStopRecording2(widget.callId!, widget.callChannel!, remoteID!);
  }
