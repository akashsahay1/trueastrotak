...
import 'package:trueastrotalk/views/call/player.dart';
...
class CallHistoryDetailScreen extends StatefulWidget {
  ...
  final int index;
  final int callType;
  CallHistoryDetailScreen({a, o, required this.astrologerId, required this.astrologerProfile, required this.index, required this.callType}) : super();

  @override
  State<CallHistoryDetailScreen> createState() => _CallHistoryDetailScreenState();
}
      ...
      child: Scaffold(
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child:
                      widget.astrologerProfile == "" || historyController.callHistoryListById.isEmpty
                          ? Image.asset(Images.deafultUser, height: 40, width: 30)
                          : CachedNetworkImage(imageUrl: '${global.imgBaseurl}${widget.astrologerProfile}', placeholder: (context, url) => const Center(child: CircularProgressIndicator()), errorWidget: (context, url, error) => Image.asset(Images.deafultUser, height: 40, width: 30)),
                ),
                const SizedBox(width: 10),
                Text(historyController.callHistoryListById.isEmpty ? "" : historyController.callHistoryListById[0].astrologerName!, style: Get.theme.primaryTextTheme.titleLarge!.copyWith(fontSize: 18, fontWeight: FontWeight.normal, color: Colors.white)),
              ],
            ),
          ),
          leading: IconButton(
        ...
        body:
            historyController.callHistoryListById.isEmpty
                ? Center(child: Text('No Details Found').tr())
                : Container(
                  width: Get.width,
                  ...
                  child: ListView(
                    children: [
                      Card(
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text('Appoinment Schedule:', style: Get.textTheme.titleMedium!.copyWith(fontWeight: FontWeight.bold)).tr(),
                              Text('${tr("Expert Name:")} ${historyController.callHistoryListById[0].astrologerName}'),
                              Text("${DateFormat("dd MMM yy, hh:mm a").format(DateTime.parse(historyController.callHistoryListById[0].createdAt.toString()))}"),
                              Text('${tr("Duration:")} ${historyController.callHistoryListById[0].totalMin ?? 0} ${tr("Minutes")}'),
                              Text('${tr("Price")}: ${global.getSystemFlagValueForLogin(global.systemFlagNameList.currency)} ${historyController.callHistoryListById[0].callRate}'),
                              Text('${tr("Deduction")}: ${global.getSystemFlagValueForLogin(global.systemFlagNameList.currency)} ${historyController.callHistoryListById[0].deduction}'),
                            ],
                          ),
                        ),
                      ),
                      ...
                      widget.callType == 10
                          ? GestureDetector(
                            ...
                            child: Container(
                              height: 6.h,
                              width: 40.w,
                              decoration: BoxDecoration(borderRadius: BorderRadius.circular(5.h), border: Border.all(color: Colors.pink, width: 1)),
                              child: Center(child: Text('Play', style: Theme.of(context).primaryTextTheme.titleSmall!.copyWith(fontWeight: FontWeight.w600))),
                            ),
                          )
                          : SizedBox.shrink(),
                      ...
                      InkWell(
                        onTap: () {
                          Get.dialog(
                            AlertDialog(
                              ...
                              content: addReviewWidget(historyController.callHistoryListById.isEmpty ? "" : historyController.callHistoryListById[0].astrologerName!, "astrologerProfile", context),
                            ),
                          );
                        ...
                        child: Container(alignment: Alignment.center, child: Card(child: Padding(padding: const EdgeInsets.all(9.0), child: Text("Add your Review")))),
                      ),
                    ],
                  ),
                ),
          ...
          child: Column(
            children: [
              ...
              widget.astrologerProfile == ""
                  ? Center(child: CircleAvatar(radius: 33, child: CircleAvatar(radius: 30, backgroundColor: Colors.white, child: Image.asset(Images.deafultUser, fit: BoxFit.fill, height: 40))))
                  : Center(
              ...
              Row(
                children: [
                  !chatController.isPublic
                      ...
                      : CircleAvatar(radius: 22, child: CircleAvatar(backgroundColor: Colors.white, backgroundImage: AssetImage(Images.deafultUser))),
                  const SizedBox(width: 10),
                  !chatController.isPublic ? Text(splashController.currentUser!.name ?? "Anonymous").tr() : Text("Anonymous").tr(),
                ],
              ),
              Row(
                ...
                children: [
                  ...
                  SizedBox(width: Get.width * 0.5, child: Text('Hide my name from all public reviews', style: Get.textTheme.titleMedium!.copyWith(fontSize: 12)).tr()),
                ],
              ),
              Center(
                child: RatingBar(
              ...
              SizedBox(
                ...
                child: Padding(
                  padding: const EdgeInsets.all(8.0),
                  child: TextButton(
                    style: ButtonStyle(padding: MaterialStateProperty.all(EdgeInsets.all(10)), backgroundColor: MaterialStateProperty.all(Get.theme.primaryColor), shape: MaterialStateProperty.all(RoundedRectangleBorder(borderRadius: BorderRadius.circular(30)))),
                    onPressed: () async {
                      print('submit');
                      if (chatController.rating == 0) {
                        global.showToast(message: 'Rate Astrologer', textColor: global.textColor, bgColor: global.toastBackGoundColor);
                      } else if (chatController.reviewController.text == "") {
                        global.showToast(message: 'Enter Review', textColor: global.textColor, bgColor: global.toastBackGoundColor);
                      } else {
                        if (chatController.reviewData.isNotEmpty) {
                          global.showOnlyLoaderDialog(context);
                          await chatController.updateReview(chatController.reviewData[0].id!, widget.astrologerId);
                          global.hideLoader();
                        } else {
                          global.showOnlyLoaderDialog(context);
                    ...
                    child: Text('Submit', textAlign: TextAlign.center, style: Get.theme.primaryTextTheme.titleMedium!.copyWith(fontSize: 12, fontWeight: FontWeight.w500, color: Colors.white)).tr(),
                  ),
                ),
              ),
            ],
