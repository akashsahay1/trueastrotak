...
class CallIntakeFormScreen extends StatefulWidget {
  ...
  CallIntakeFormScreen({Key? key, this.reportType, this.isFreeAvailable = false, required this.astrologerName, required this.astrologerId, required this.type, required this.rate, required this.astrologerProfile}) : super(key: key);
...
class _CallIntakeFormScreenState extends State<CallIntakeFormScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('${widget.type} ${tr("Intake Form")}', style: Get.theme.primaryTextTheme.titleLarge!.copyWith(fontSize: 15, fontWeight: FontWeight.normal, color: Colors.white)).tr(),
        leading: IconButton(
          onPressed: () => Get.back(),
          icon: Icon(
            kIsWeb
      ...
      body: SingleChildScrollView(
        child: Padding(
          ...
          child: GetBuilder<IntakeController>(
            builder: (c) {
              return Column(
                ...
                children: [
                  SizedBox(height: 15),
                  TextFieldWidget(controller: callIntakeController.nameController, focusNode: callIntakeController.namefocus, inputFormatter: [FilteringTextInputFormatter.allow(RegExp("[a-zA-Z ]"))], labelText: 'Name'),
                  Row(
                        ...
                        child: Padding(
                          ...
                          child: Theme(
                            ...
                            child: InternationalPhoneNumberInput(
                              ...
                              searchBoxDecoration: InputDecoration(border: OutlineInputBorder(borderRadius: BorderRadius.all(Radius.circular(2.w)), borderSide: const BorderSide(color: Colors.white)), hintText: "Search", hintStyle: const TextStyle(color: Colors.black)),
                  ...
                  Row(
                    ...
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      TextFieldLabelWidget(label: 'Gender'),
                      Flexible(
                        flex: 1,
                        child: RadioListTile(
                          title: Text("Male").tr(),
                  ...
                  InkWell(
                    onTap: () async {
                      ...
                      var datePicked = await DatePicker.showSimpleDatePicker(
                        ...
                        itemTextStyle: Get.theme.textTheme.titleMedium!.copyWith(color: Colors.black, fontSize: 15, fontWeight: FontWeight.w400, letterSpacing: 0),
                    ...
                    child: IgnorePointer(child: TextFieldWidget(controller: callIntakeController.dobController, labelText: tr('Date of Birth'))),
                  ),
                  InkWell(
                    onTap: () async {
                      callIntakeController.namefocus.unfocus();
                      ...
                      final time = await showTimePicker(
                        ...
                        builder: (context, child) {
                          return Theme(data: ThemeData(colorScheme: ColorScheme.light(primary: Get.theme.primaryColor, onSurface: Colors.black)), child: child ?? SizedBox());
                        },
                      );
                    ...
                    child: Column(
                      children: [
                        IgnorePointer(child: TextFieldWidget(controller: callIntakeController.birthTimeController, labelText: tr('Time of Birth'))),
                        Row(
                          children: [
                            Checkbox(
                  ...
                  InkWell(
                    ...
                    child: IgnorePointer(child: TextFieldWidget(controller: callIntakeController.placeController, labelText: tr('Place of Birth'))),
                  ),
                  TextFieldLabelWidget(label: 'Marital Status (Optional)'),
                  DropDownWidget(item: ['single', 'Married', 'Divorced', 'Separated', 'Widowed'], hint: tr('Select Marital Status'), callId: 1),
                  const SizedBox(height: 6),
                  TextFieldWidget(controller: callIntakeController.ocupationController, focusNode: callIntakeController.occupationfocus, labelText: 'Occupation (Optional)', inputFormatter: [FilteringTextInputFormatter.allow(RegExp("[a-zA-Z ]"))]),
                  SizedBox(height: 6),
                  widget.isFreeAvailable == true ? SizedBox() : Text("How Many Minutes you want to talk?"),
                  widget.isFreeAvailable == true ? SizedBox() : SizedBox(height: 10),
                  ...
                  DropDownWidget(item: ['Study', 'Future', 'Past'], hint: tr('Select Topic of Concern'), callId: 3),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.start,
                    children: [
                      Checkbox(
                      ...
                      Text("Enter Partner's Details", style: Get.textTheme.titleMedium!.copyWith(fontSize: 12, decoration: TextDecoration.underline)).tr(),
                    ],
                  ),
                  if (callIntakeController.isEnterPartnerDetails)
                    if (callIntakeController.isEnterPartnerDetails) TextFieldWidget(controller: callIntakeController.partnerNameController, labelText: "Partner's Name", focusNode: callIntakeController.partnerNamefocus, inputFormatter: [FilteringTextInputFormatter.allow(RegExp("[a-zA-Z ]"))]),
                  if (callIntakeController.isEnterPartnerDetails)
                    if (callIntakeController.isEnterPartnerDetails)
                      InkWell(
                        onTap: () async {
                          ...
                          var datePicked = await DatePicker.showSimpleDatePicker(
                            ...
                            lastDate: DateTime.now(),
                            dateFormat: "dd-MM-yyyy",
                            itemTextStyle: Get.theme.textTheme.titleMedium!.copyWith(fontSize: 15, fontWeight: FontWeight.w400, letterSpacing: 0),
                        ...
                        child: IgnorePointer(child: TextFieldWidget(controller: callIntakeController.partnerDobController, labelText: "Partner's DOB")),
                      ),
                  if (callIntakeController.isEnterPartnerDetails)
                    if (callIntakeController.isEnterPartnerDetails)
                      InkWell(
                        ...
                        child: IgnorePointer(
                          child: Column(
                            children: [
                              TextFieldWidget(controller: callIntakeController.partnerBirthController, labelText: "Partner's Time of Birth"),
                              Row(
                                children: [
                                  Checkbox(
                                    checkColor: Colors.white,
                  ...
                  if (callIntakeController.isEnterPartnerDetails)
                    if (callIntakeController.isEnterPartnerDetails)
                      InkWell(
                        ...
                        child: IgnorePointer(child: TextFieldWidget(controller: callIntakeController.partnerPlaceController, labelText: tr('Place of Birth'))),
                      ),
      ...
      bottomSheet: GetBuilder<IntakeController>(
        builder: (intakeController) {
          return CustomBottomButton(
            onTap: () async {
              ...
              if (charge * time[selectTime] <= global.splashController.currentUser!.walletAmount! || bottomNavigationController.astrologerbyId[0].isFreeAvailable == true) {
                bool isvalid = intakeController.isValidData();
                print(isvalid);
                if (!isvalid) {
                  global.showToast(message: intakeController.errorText, textColor: global.textColor, bgColor: global.toastBackGoundColor);
                } else {
                    ...
                    if (widget.isFreeAvailable == true) {
                      ...
                      if (intakeController.isAddNewRequestByFreeuser == true) {
                        ...
                        if (widget.type == "Call" || widget.type == "Videocall") {
                          await callController.sendCallRequest(widget.astrologerId, true, widget.type, callIntakeController.freedefaultTime.toString());
                        } else {
                          ChatController chatController = Get.find<ChatController>();
                          ...
                          await chatController.sendChatRequest(widget.astrologerId, true, callIntakeController.freedefaultTime.toString());
                        }
                      } else {
                        global.showToast(message: 'You can not join multiple offers at same time', textColor: global.textColor, bgColor: Colors.white);
                      }
                    } else {
                      if (widget.type == "Call" || widget.type == "Videocall") {
                        await callController.sendCallRequest(widget.astrologerId, false, widget.type, (int.parse(time[selectTime].toString().split(".").first) * 60).toString());
                      } else {
                        ChatController chatController = Get.find<ChatController>();
                        DropDownController dropDownController = Get.find<DropDownController>();
                        await chatController.sendMessage(
                        ...
                        await chatController.sendChatRequest(widget.astrologerId, false, (int.parse(time[selectTime].toString().split(".").first) * 60).toString());
                      }
                    }
                    global.hideLoader();
                    dialogForchat(context);
                  } else {
                    global.showToast(message: tr('Please verify your phone number'), textColor: global.textColor, bgColor: global.toastBackGoundColor);
                  }
                }
              } else {
                ...
                global.hideLoader();
                openBottomSheetRechrage(context, (charge * time[selectTime]).toString(), '${widget.type}', '${bottomNavigationController.astrologerbyId[0].name}', time[selectTime].toString());
              }
            },
            title: '${tr("Start")} ${widget.type} ${tr("with")} ${widget.astrologerName}',
          );
        },
      ),
    );
                              ...
                              Row(
                                ...
                                children: [
                                  SizedBox(
                                    ...
                                    child:
                                        minBalance != ''
                                            ? Text('Minimum balance of $min minutes(${global.getSystemFlagValueForLogin(global.systemFlagNameList.currency)} $minBalance) is required to start $type with $astrologer ', style: TextStyle(fontWeight: FontWeight.w500, color: Colors.red)).tr()
                                            : const SizedBox(),
                                  ),
                                  GestureDetector(
  ...
  dialogForchat(BuildContext context) {
      ...
      builder: (BuildContext context) {
        return Container(
          ...
          child: AlertDialog(
            ...
            title: Column(
              children: [
                Container(padding: EdgeInsets.all(1.w), child: Center(child: Text("You're all set!", style: Get.theme.textTheme.displayLarge!.copyWith(color: Colors.black, fontSize: 18.sp, fontWeight: FontWeight.w600, fontStyle: FontStyle.normal)).tr())),
                SizedBox(height: 4),
                Container(
                        ...
                        children: [
                          CircleAvatar(
                            ...
                            child: CachedNetworkImage(
                              ...
                              errorWidget: (context, url, error) => CircleAvatar(radius: 28, backgroundImage: AssetImage(Images.deafultUser)),
                            ),
                          ),
                          SizedBox(height: 2),
                          Text(splashController.currentUser!.name!.isEmpty ? "User" : "${splashController.currentUser!.name}", style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.normal, color: Colors.black)),
                        ],
                      ),
                      Center(child: Text("••••", style: TextStyle(color: Colors.pink.shade400, fontSize: 30.sp))),
                      Column(
                        children: [
                          CircleAvatar(
                            maxRadius: 30,
                          ...
                          SizedBox(height: 2),
                          Text("${widget.astrologerName}", textAlign: TextAlign.right, style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.normal, color: Colors.black)).tr(),
                        ],
                      ),
                    ],
                  ),
                ...
                Container(
                  ...
                  decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(4.w), border: Border.all(color: Colors.grey, width: 0.3)),
                  child: Column(
                    children: [
                      Container(
                        ...
                        // height: 4.h,
                        width: MediaQuery.of(context).size.width,
                        decoration: BoxDecoration(color: Colors.grey.shade400, borderRadius: BorderRadius.only(topLeft: Radius.circular(4.w), topRight: Radius.circular(4.w)), border: Border.all(color: Colors.grey, width: 0.5)),
                        child: Align(alignment: Alignment.centerLeft, child: Text("What's Next! ", style: TextStyle(color: Colors.black, fontSize: 16.sp, fontWeight: FontWeight.bold))),
                      ),
                      Column(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Text('You will connect with ${widget.astrologerName} after the astrologer accepts your request', style: TextStyle(fontSize: 17.sp, fontWeight: FontWeight.normal, color: Colors.black)).tr(),
                          ),
                          SizedBox(height: 2),
                          // Container(
                          //   padding: EdgeInsets.symmetric(horizontal: 2.w),
    ...
    //                             padding: EdgeInsets.symmetric(horizontal: 2.w),
    //                             child: Text(
    //                               'True Astrotalk will try to answer at least one question in this 5 mins session',
    //                               style: TextStyle(
    //                                   fontSize: 15.sp,
    //                                   fontWeight: FontWeight.normal,
    //                                   color: Colors.black),
