...
class _PdfViewerPageState extends State<PdfViewerPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(backgroundColor: Colors.white, actions: [IconButton(icon: Icon(Icons.download), onPressed: _downloadPdf)]),
  ...
  getPdfDownloadUrl(String prefixurl) async {
    ...
    try {
      final storageRef = FirebaseStorage.instance.ref().child(Uri.decodeFull(prefixurl));
  ...
  void _downloadPdf() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('PDF downloaded successfully!'), duration: Duration(seconds: 2)));
      } else {
  ...
  void showDownloadCompleteNotification() async {
    AndroidNotificationDetails androidDetails = AndroidNotificationDetails('channel.id', 'channel.name', importance: Importance.max, priority: Priority.high, icon: "@mipmap/ic_launcher", playSound: true);
    const DarwinNotificationDetails iOSDetails = DarwinNotificationDetails();

    NotificationDetails platformChannelSpecifics = NotificationDetails(android: androidDetails, iOS: iOSDetails);

    await flutterLocalNotificationsPlugin.show(0, 'Download Complete', 'Your PDF has been downloaded successfully!', platformChannelSpecifics);
  }

  void _initializeNotifications(String locationpath) async {
    final AndroidInitializationSettings initializationSettingsAndroid = AndroidInitializationSettings('@mipmap/ic_launcher');
    final DarwinInitializationSettings initializationSettingsDarwin = DarwinInitializationSettings(defaultPresentBadge: true, requestSoundPermission: true, requestBadgePermission: true, defaultPresentSound: true);
    final InitializationSettings initialSetting = InitializationSettings(android: initializationSettingsAndroid, iOS: initializationSettingsDarwin);
