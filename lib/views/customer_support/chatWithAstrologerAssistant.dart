  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Container(
        ...
        child:
            astrologerAssistantController.assistantList.isEmpty
                ? Center(child: FittedBox(child: Text("You have not texted any astrologer's assistant yet").tr()))
                          ...
                          onLongPress: () async {
                            Get.dialog(
                              AlertDialog(
                                ...
                                title: Text("Are you sure you want delete chat with Astrologer Assistant?", style: Get.textTheme.titleMedium).tr(),
                                content: Row(
                                  children: [
                                    Expanded(
                                      flex: 4,
                                ...
                                children: [
                                  Container(
                                    ...
                                    child: CircleAvatar(
                                      radius: 25,
                                      child:
                                          astrologerAssistantController.assistantList[index].profileImage == ""
                                              ? CircleAvatar(radius: 24, backgroundColor: Colors.white, child: Image.asset(Images.deafultUser, fit: BoxFit.cover, height: 50, width: 40))
                                              : CachedNetworkImage(
                                                imageUrl: '${global.imgBaseurl}${astrologerAssistantController.assistantList[index].profileImage}',
                                                imageBuilder: (context, imageProvider) => CircleAvatar(radius: 24, backgroundColor: Colors.white, child: Image.network(fit: BoxFit.cover, height: 50, width: 40, '${global.imgBaseurl}${astrologerAssistantController.assistantList[index].profileImage}')),
                                  ...
                                  Column(
                                    ...
                                    children: [
                                      ...
                                      Text(astrologerAssistantController.assistantList[index].lastMessage ?? '', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold)),
                                      Text(DateFormat('dd MMM yyyy , hh:mm a').format(astrologerAssistantController.assistantList[index].lastMessageTime ?? DateTime.now()), style: TextStyle(color: Colors.grey)),
                                    ],
                                  ),
                                ],
                              ),
