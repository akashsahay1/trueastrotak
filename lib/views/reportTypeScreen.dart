...
class ReportTypeScreen extends StatelessWidget {
  ...
  ReportTypeScreen({Key? key, required this.astrologerId, required this.astrologerName}) : super(key: key);
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text('Select Report type', style: Get.theme.primaryTextTheme.titleLarge!.copyWith(fontSize: 15, fontWeight: FontWeight.normal, color: Colors.white)).tr(),
          leading: IconButton(
            onPressed: () {
              reportController.searchReportController.clear();
              Get.back();
                      ...
                      return Column(
                        ...
                        children: [
                                  ...
                                  Expanded(
                                    child: TextField(
                                      ...
                                      decoration: InputDecoration(
                                        ...
                                        isDense: true,
                                        border: new UnderlineInputBorder(borderSide: new BorderSide(color: Get.theme.primaryColor)),
                                        enabledBorder: new UnderlineInputBorder(borderSide: new BorderSide(color: Get.theme.primaryColor)),
                          ...
                          reportController.reportTypeList.isEmpty
                              ? SizedBox(height: Get.height * 0.8, child: Center(child: Text('Result Not Found').tr()))
                              : ListView.builder(
...
class CustomContainer extends StatelessWidget {
  const CustomContainer({Key? key, required this.title, required this.image, required this.desc, required this.astrologerId, required this.astrologerName, required this.reportId}) : super(key: key);
  final String title;
  final String image;
  final String desc;
  final int astrologerId;
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () {
        Get.to(() => ReportInTakeForm(reportId: reportId, reportType: title, astrologerId: astrologerId, astrologerName: astrologerName));
      },
      child: Container(
        width: double.infinity,
        decoration: BoxDecoration(color: Color.fromARGB(255, 247, 236, 235), borderRadius: BorderRadius.only(bottomRight: Radius.circular(10.0), bottomLeft: Radius.circular(10.0))),
        child: Column(
          mainAxisAlignment: MainAxisAlignment.start,
          crossAxisAlignment: CrossAxisAlignment.start,
          mainAxisSize: MainAxisSize.min,
          children: [
            image == ""
                ? Image.asset(Images.yearlyImage, fit: BoxFit.cover, height: 200, width: double.infinity)
                : CachedNetworkImage(
                  imageUrl: '${global.imgBaseurl}$image',
                  imageBuilder: (context, imageProvider) => Image.network('${global.imgBaseurl}$image', fit: BoxFit.cover, height: 200, width: double.infinity),
                  placeholder: (context, url) => const Center(child: CircularProgressIndicator()),
                  errorWidget: (context, url, error) => Image.asset(Images.yearlyImage, fit: BoxFit.cover, height: 200, width: double.infinity),
                ),
            Padding(padding: const EdgeInsets.only(top: 8.0, bottom: 6, left: 8.0), child: Text(title, style: Get.textTheme.titleMedium!.copyWith(fontSize: 15, fontWeight: FontWeight.bold)).tr()),
            FutureBuilder(
