...
class KundliBornPlaceWidget extends StatefulWidget {
  ...
  const KundliBornPlaceWidget({Key? key, required this.kundliController}) : super(key: key);
    ...
    return Column(
      ...
      children: [
        ...
        Flexible(
          flex: 1,
          child: RadioListTile(
            title: widget.kundliController.pdfPriceData!.isFreeSession == true ? Text("Free Kundali").tr() : Text("Basic Kundali (₹${widget.kundliController.pdfPriceData!.recordList!.small})").tr(),
            value: "small",
            groupValue: type,
            dense: true,
        ...
        Flexible(
          flex: 1,
          child: RadioListTile(
            title: Text("Detailed Kundali (₹${widget.kundliController.pdfPriceData!.recordList!.medium})").tr(),
            value: "medium",
            groupValue: type,
            dense: true,
            activeColor: Get.theme.primaryColor,
        ...
        Flexible(
          flex: 1,
          child: RadioListTile(
            title: Text("Full-fledged Kundali (₹${widget.kundliController.pdfPriceData!.recordList!.large})").tr(),
            value: "large",
            groupValue: type,
            dense: true,
            activeColor: Get.theme.primaryColor,
        ...
        SizedBox(height: 10),
        Text("Select Your Kundali Language", style: TextStyle(color: Colors.black, fontWeight: FontWeight.w500, fontSize: 18.sp)),
        SizedBox(height: 10),
        DropDownWidget(item: ['English', 'Tamil', 'Kannada', 'Telugu', 'Hindi', 'Malayalam', 'Spanish', 'French'], hint: tr('Select Your Language'), callId: 4),
        SizedBox(height: 15),
        SizedBox(
          ...
          child: TextButton(
            ...
            onPressed: () async {
              if (type == "small"
                  ? (widget.kundliController.pdfPriceData!.isFreeSession == true ? false : global.splashController.currentUser!.walletAmount! < int.parse(widget.kundliController.pdfPriceData!.recordList!.small.toString()))
                  : type == "medium"
                  ? (global.splashController.currentUser!.walletAmount! < int.parse(widget.kundliController.pdfPriceData!.recordList!.medium.toString()))
                  : type == "large"
                  ? (global.splashController.currentUser!.walletAmount! < int.parse(widget.kundliController.pdfPriceData!.recordList!.large.toString()))
                  : false) {
                openBottomSheetRechrage(context, type == "small" ? widget.kundliController.pdfPriceData!.recordList!.small.toString() : (type == "medium" ? "${widget.kundliController.pdfPriceData!.recordList!.medium.toString()}" : widget.kundliController.pdfPriceData!.recordList!.large.toString()));
              } else {
                if (widget.kundliController.birthKundliPlaceController.text == "") {
                  global.showToast(message: 'Please select birth place', textColor: global.textColor, bgColor: global.toastBackGoundColor);
                } else {
                  widget.kundliController.updateIcon(widget.kundliController.initialIndex);
                  global.showOnlyLoaderDialog(context);
                  await widget.kundliController.addKundliData(
                    type,
                    type.toString() == "large"
                        ? int.parse(widget.kundliController.pdfPriceData!.recordList!.large.toString())
                        : (type.toString() == "medium"
                            ? int.parse(widget.kundliController.pdfPriceData!.recordList!.medium.toString())
                            : (type.toString() == "small" ? (widget.kundliController.pdfPriceData!.isFreeSession == true ? 0 : int.parse(widget.kundliController.pdfPriceData!.recordList!.small.toString())) : 0)),
                  );
            ...
            child: Text('Submit', textAlign: TextAlign.center, style: TextStyle(color: Colors.white)).tr(),
          ),
        ),
      ],
    );
                              ...
                              Row(
                                ...
                                children: [
                                  SizedBox(
                                    ...
                                    child: minBalance != '' ? Text('Minimum balance (${global.getSystemFlagValueForLogin(global.systemFlagNameList.currency)} $minBalance) is required to Create Kundali', style: TextStyle(fontWeight: FontWeight.w500, color: Colors.red)).tr() : const SizedBox(),
                                  ),
                                  GestureDetector(
